...
export { fetchGoogleBooks, fetchOpenLibrary, fetchLoc }
export * from './types'
...
export async function fetchBookByIsbn(isbn: string): Promise<{ data: BookMetadata | null, meta: unknown }> {
  ...
  const meta = {
    ...
    individualDurations: successfulResults.reduce((acc, r) => ({ ...acc, [r.source]: r.durationMs }), {})
  }
  ...
  const pick = <K extends keyof BookMetadata>(key: K, ...sources: (BookMetadata | undefined | null)[]): BookMetadata[K] => {
  ...
  if (ol?.description && google?.description && ol.description.length > google.description.length) {
...
async function measureFetch(source: string, fetcher: () => Promise<BookMetadata | null>): Promise<MetadataFetchResult> {
