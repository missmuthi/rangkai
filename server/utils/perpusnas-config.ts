...
const parseEnvList = (value?: string) =>
  (value || "")
    ...
    .map((url, idx) => ({
      ...
      url,
      metadataPrefix:
        url.includes("perpusnas") || url.includes("inlislite")
          ? "marcxml"
          : "oai_dc",
      priority: idx + 1,
      status: "active" as const,
    })) as PerpusnasEndpoint[];
 ...
 */
export function getEndpointsByPriority(
  ...
  return includeAggregators
    ? combined
    : combined.filter((ep) => !ep.aggregator);
}
