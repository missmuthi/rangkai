...
export function useScanner() {
  ...
  const checkPermission = async () => {
    ...
    console.log(
      "[Scanner] navigator exists:",
      typeof navigator !== "undefined"
    );
    console.log(
      "[Scanner] navigator.permissions exists:",
      typeof navigator !== "undefined" && !!navigator.permissions
    );
    console.log(
      "[Scanner] navigator.permissions.query exists:",
      typeof navigator !== "undefined" && !!navigator.permissions?.query
    );
    console.log(
      "[Scanner] navigator.mediaDevices exists:",
      typeof navigator !== "undefined" && !!navigator.mediaDevices
    );
    console.log(
      "[Scanner] navigator.mediaDevices.getUserMedia exists:",
      typeof navigator !== "undefined" && !!navigator.mediaDevices?.getUserMedia
    );

    if (typeof navigator === "undefined" || !navigator.permissions?.query) {
      console.log(
        "[Scanner] Permission API not available, returning unsupported"
      );
  ...
  const updateTorchSupport = () => {
    ...
    const capabilities =
      scanner.value.getRunningTrackCapabilities() as MediaTrackCapabilities & {
        torch?: boolean;
      };
    if (!capabilities) return;
    const supported =
      "torch" in capabilities ? Boolean(capabilities.torch) : false;
  ...
  const setTorch = async (enabled: boolean) => {
    ...
    try {
      await scanner.value.applyVideoConstraints({
  ...
  async function startScanner(
    ...
    try {
      ...
      if (typeof window !== "undefined" && window.__SCANNER_DISABLED__) {
      ...
      if (permission !== "granted") {
        console.log(
          "[Scanner] Requesting camera permission via getUserMedia..."
        );
      ...
      cameras.forEach((cam, i) => {
        console.log(
          `[Scanner]   Camera ${i}: ${cam.label} (${cam.id.slice(0, 8)}...)`
        );
      });
      ...
      if (options?.deviceId) {
        attempts.push({
          config: options.deviceId,
          label: `deviceId: ${options.deviceId}`,
        });
      }
      attempts.push(
        {
      ...
      if (!started) {
        try {
          ...
          console.log(
            "[Scanner] Available cameras for fallback:",
            cameras.length
          );
          ...
          if (fallbackList.length > 0) {
            ...
            const fallbackCamera =
              fallbackList.find((cam) => cam.id === options?.deviceId) ||
              fallbackList[0];
            console.log(
              "[Scanner] Retrying with camera ID:",
              fallbackCamera!.id
            );
            started = await tryStart(
              fallbackCamera!.id,
              `deviceId: ${fallbackCamera!.id}`
            );
          }
        } catch (cameraListErr) {
          lastError = cameraListErr;
          console.error(
            "[Scanner] Failed to list cameras for fallback:",
            cameraListErr
          );
        }
      }
