...
async function handleAiClean() {
  ...
  try {
    ...
    const savePromise = scanId.value
      ? $fetch<{ id: number; [key: string]: unknown }>(
          `/api/scans/${scanId.value}`,
          { method: "PATCH", body: cleaned }
        )
      : $fetch<{ id: number; [key: string]: unknown }>("/api/scans", {
          method: "POST",
          body: cleaned,
        });
    ...
    savePromise
      .then((result) => {
        if (!scanId.value && result?.id) scanId.value = String(result.id);
    ...
    // Type guard for common error shapes
    const isErrorWithData = (
      e: unknown
    ): e is { data: { message: string } } => {
      if (!e || typeof e !== "object" || !("data" in e)) return false;
      const obj = e as { data: unknown };
      return (
    ...
    const isErrorWithStatus = (e: unknown): e is { statusMessage: string } =>
...
<template>
  <div class="container mx-auto p-4 max-w-5xl min-h-screen py-8">
    ...
    <div class="flex items-center justify-between mb-8">
      <div class="flex items-center gap-4">
        ...
        <ClientOnly>
          <span
            v-if="isSaved"
            class="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-200 dark:border-green-800"
          >
            ✓ Saved to Library
          </span>
          <template #fallback>
            <div
              class="hidden sm:block h-6 w-28 bg-muted/40 animate-pulse rounded-full"
            />
          </template>
        </ClientOnly>
      </div>
      ...
      <ClientOnly>
        <div class="flex items-center gap-2">
          ...
          <Button
            ...
            :class="
              [
                ...
                !book.isAiEnhanced && !isCleaning && cooldown === 0
                  ? 'shadow-[0_0_20px_rgba(99,102,241,0.5),0_0_40px_rgba(99,102,241,0.2)] animate-pulse'
                  : '',
              ].join(' ')
            "
          ...
          >
            <Sparkles
              class="w-4 h-4"
              :class="{ 'animate-pulse': isCleaning }"
            />
    ...
    <div
      class="fixed bottom-4 right-4 z-50 p-4 bg-black/80 text-white text-xs font-mono rounded shadow-lg pointer-events-auto"
    >
      <p>Loaded: {{ !!book }}</p>
      <p>Loading: {{ loading }}</p>
      ...
      <p>ISBN: {{ isbn }}</p>
      <button
        class="mt-2 underline text-yellow-400"
    ...
    <div
      v-if="showDebug"
      class="fixed inset-0 z-50 bg-black/90 text-green-400 p-8 overflow-auto font-mono text-xs"
    >
      <button
        class="absolute top-4 right-4 text-white text-lg"
        @click="showDebug = false"
      >
        ✕
      </button>
      <pre>{{
        JSON.stringify({ book, loading, error, cleanError }, null, 2)
      }}</pre>
    </div>
    ...
    <div
      ...
      class="p-8 bg-white dark:bg-gray-900 rounded-xl border border-indigo-100 dark:border-indigo-900/50 shadow-lg space-y-6 max-w-2xl mx-auto my-12"
    >
      <div
        class="flex items-center gap-3 text-indigo-600 dark:text-indigo-400 animate-pulse"
      >
      ...
      <div class="space-y-3">
        ...
        <Skeleton
          class="h-32 w-full bg-indigo-50 dark:bg-indigo-900/20 rounded-lg"
        />
      </div>
    </div>
    ...
    <!-- AI Error Display -->
    <div
      v-else-if="cleanError"
      class="p-6 bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 rounded-lg border border-amber-200 dark:border-amber-900/30 max-w-2xl mx-auto my-12"
    >
      <!-- ... error content ... -->
      <p>{{ cleanError }}</p>
    ...
    <div
      v-else-if="error"
      class="p-6 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 rounded-lg border border-red-100 dark:border-red-900/30"
    >
    ...
    <div
      v-else-if="book"
      class="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 md:p-8"
    >
      <!-- Mobile Saved Badge (ClientOnly) -->
      <ClientOnly>
        <div
          v-if="isSaved"
          class="sm:hidden mb-6 flex items-center justify-center"
        >
          <span
            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-medium bg-green-50 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-100 dark:border-green-900/50"
          >
            ✓ Saved to Library
          </span>
        </div>
    ...
    <div class="mt-12 pt-8 border-t border-gray-200 dark:border-gray-800">
      ...
      <div
        v-if="showDebug"
        class="mt-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-auto max-h-96 font-mono text-xs text-gray-700 dark:text-gray-300"
      >
        <div class="flex justify-between items-center mb-2">
          <span class="font-bold">Raw Book Data:</span>
          <button class="text-indigo-600 hover:underline" @click="copyDebug">
            Copy JSON
          </button>
        </div>
