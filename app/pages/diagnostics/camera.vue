...
async function runDiagnostics() {
  ...
  addResult({
    ...
    message: window.isSecureContext ? 'Running in secure context' : 'NOT secure context - camera will not work',
  ...
  addResult({
    ...
    message: typeof navigator !== 'undefined' ? 'Navigator available' : 'Navigator not available'
  })
  ...
  addResult({
    ...
    details: hasMediaDevices ? `getUserMedia: ${!!navigator.mediaDevices.getUserMedia}` : undefined
  })

  // 4. Check Permissions API
  const hasPermissionsAPI = typeof navigator !== 'undefined' && !!navigator.permissions?.query
  addResult({
    name: 'Permissions API',
    status: hasPermissionsAPI ? 'success' : 'warning',
    message: hasPermissionsAPI ? 'Permissions API available' : 'Permissions API not available (will try direct access)',
  })
  ...
  if (hasPermissionsAPI) {
    try {
      ...
      addResult({
        ...
        details: permission.state === 'denied' 
  ...
  if (hasMediaDevices) {
    try {
      ...
      addResult({
        ...
        details: cameras.map((c, i) => `${i + 1}. ${c.label || 'Unknown'} (${c.deviceId.slice(0, 8)}...)`).join('\n') || 'No labels available (permission not granted yet)'
      })
    } catch (err) {
      addResult({
        name: 'Available Cameras',
  ...
  if (hasMediaDevices && navigator.mediaDevices.getUserMedia) {
    try {
      ...
      results.value[results.value.length - 1] = {
        ...
        details: `Tracks: ${stream.getTracks().length}, Active: ${stream.active}`
      }
      ...
      if (track) {
        ...
        addResult({
          ...
          message: capabilities.torch ? 'Torch is supported' : 'Torch not supported on this device'
        })
      }
    } catch (err) {
      results.value[results.value.length - 1] = {
        ...
        details: err instanceof DOMException
          ? `${err.name}: ${err.message}${err.name === 'NotAllowedError' ? '\nHint: Allow camera access for this site in your browser settings, then refresh.' : ''}`
  ...
  // 8. Check html5-qrcode availability
  try {
    const { Html5Qrcode } = await import('html5-qrcode')
    addResult({
      name: 'QR Scanner Library',
      status: 'success',
      message: 'html5-qrcode loaded successfully',
      details: `Html5Qrcode available: ${!!Html5Qrcode}`
    })
  } catch (err) {
    addResult({
      name: 'QR Scanner Library',
      status: 'error',
      message: 'Failed to load html5-qrcode',
      details: err instanceof Error ? err.message : String(err)
    })
  }
...
<template>
  <main class="max-w-2xl mx-auto px-4 py-10 space-y-6">
    <header class="space-y-2">
      ...
      <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Camera Diagnostics</h1>
    ...
    <div class="flex gap-2 flex-wrap">
      ...
      >
        <span v-if="isRunning" class="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />
        <span v-else>🔍</span>
      ...
      >
        <span v-if="isRequestingPermission" class="h-4 w-4 animate-spin rounded-full border-2 border-black border-t-transparent" />
        <span v-else>🔓</span>
    ...
    <!-- Camera Preview -->
    <div v-if="cameraStream" class="rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700">
      <video
        ref="videoRef"
        autoplay
        playsinline
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <span 
              ...
              :class="{
                'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400': result.status === 'success',
                'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400': result.status === 'warning',
                'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400': result.status === 'error',
                'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400 animate-pulse': result.status === 'pending'
              }"
            >
            ...
            <div class="flex-1 min-w-0">
              ...
              <pre v-if="result.details" class="mt-2 text-xs text-gray-500 dark:text-gray-500 whitespace-pre-wrap font-mono bg-gray-50 dark:bg-gray-800 p-2 rounded">{{ result.details }}</pre>
            </div>
          </div>
        </div>
      </div>
    ...
    <section class="pt-6 border-t border-gray-200 dark:border-gray-700">
      <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Related Pages</h3>
      <div class="flex gap-2 flex-wrap">
        <NuxtLink 
          to="/scan/mobile" 
